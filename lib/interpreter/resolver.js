...
var func = function func(srcFile, destFile) {
    ...
    traverse(ast, {
        enter: function enter(p) {
            ...
            if (t.isAssignmentExpression(p.node) && t.isMemberExpression(p.node.left) && t.isIdentifier(p.node.left.object) && p.node.left.object.name == 'module' && t.isIdentifier(p.node.left.property) && p.node.left.property.name == 'exports' && t.isIdentifier(p.node.right)) {

                var _className = p.node.name;

                objArr.forEach(function (obj) {
                    if (obj.className == _className || obj.alias == _className) {
                        exportObj = obj;
                    }
                });
            }
